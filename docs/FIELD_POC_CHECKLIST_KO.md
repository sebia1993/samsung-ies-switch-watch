...
- [ ] 공개 Release 사용자 정의 Assets가 Agent ZIP과 Viewer ZIP 정확히 두 개뿐임
- [ ] 두 ZIP의 BUILD-MANIFEST가 엄격한 UTF-8이며 선언 파일 외의 최상위 파일·하위 폴더가 없음
...
- [ ] 2 MiB 초과·잘못된 UTF-8·읽기 중 변경·대소문자 중복 manifest가 설치 전에 거부됨
...
- [ ] Agent 신원 자료가 크기 상한을 넘거나 손상된 경우 서비스가 비정상 종료하지 않고 안전 오류로 중단함
...
- [ ] 원자 교체가 끝난 뒤 EDR이 destination 재확인만 잠시 막아도 완료된 저장을 실패로 오판하지 않음
...
- [ ] 이벤트 폭주 시 제한된 feed 안에서 같은 변경이 합쳐지고 오래된 Agent 연결 결과가 현재 상태를 덮어쓰지 않음
- [ ] 작은 Windows 작업 영역에서 대시보드가 스크롤되고 저장된 창 위치·크기가 화면 안으로 복원됨
...
- [ ] Telnet 로그인·enable·명령 수집이 각 시간·바이트 상한에서 끝나며 Latin-1/IAC 응답을 처리함
