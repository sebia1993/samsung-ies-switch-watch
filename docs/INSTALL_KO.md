...
공식 GitHub `v0.11.9-poc` Release의 Assets에서 다음 두 파일만 받습니다.
...
`0.11.9-poc`는 코드 서명되지 않은 시험판입니다. SmartScreen, EDR, AppLocker 또는 WDAC가
...
`0.11.9-poc` Agent ZIP의 Setup에서 별도의 `이전 상태 복구`를 사용하십시오. 복구 성공 뒤에는
...
확인한 뒤 `0.11.9-poc` 패키지의 `설치/업데이트`를 한 번만 다시 실행하십시오. Setup이 내부
