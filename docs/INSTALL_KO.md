...
- Agent 신원 입력과 Viewer 로컬 JSON에는 크기 상한이 있습니다. 손상·과대 파일은 이전 정상
  상태로 가장하지 않고 안전 오류로 표시합니다.
- Viewer 이벤트 전달은 제한된 큐에서 중복 변경을 합치며, 교체된 이전 Agent 연결의 늦은 결과가
  현재 화면을 덮어쓰지 않습니다. 작은 화면에서는 스크롤과 화면 안쪽 창 복원을 사용합니다.
- 두 Setup은 BUILD-MANIFEST를 엄격한 UTF-8과 2 MiB 상한으로 읽고 읽는 도중 변경, 선언
  크기·SHA-256, Windows 대소문자 중복, 실제 파일 집합과 하위 폴더를 확인합니다.
- Viewer Setup의 파일 이동·정리는 최대 횟수와 취소가 있는 재시도로 일시적 EDR 잠금을
  흡수합니다. commit 전 새 Viewer가 손상된 경우 해당 세대를 격리하고 검증된 이전 버전을
  복구·재검증합니다.

Agent API v4, Viewer 저장 형식과 보안 경계는 바뀌지 않았습니다. 이 동작은 Mock·fixture와
로컬 패키지로 검증했으며 실제 Windows SCM, 사내 EDR/백신, 원격 방화벽·라우팅과 삼성 스위치
펌웨어는 현장 POC에서 별도로 확인해야 합니다.
...
서비스가 `START_PENDING`이면 Setup은 같은 시작 요청을 다시 보내지 않고 기존 전환을 제한 시간
안에서 기다립니다. `STOP_PENDING`이면 중지가 완료될 때까지 기다린 뒤 한 번만 시작합니다.
대기 시간은 시스템 시각 변경의 영향을 받지 않는 단조 시계로 계산하며, 제한을 넘기면
`SETUP_SERVICE_START_FAILED`로 명확히 중단합니다.
...
- 작업 기록은 같은 폴더의 임시 파일을 완전히 기록한 뒤 원자적으로 교체합니다. 교체가 완료된
  뒤 백신·EDR이 임시 파일 정리만 잠시 막더라도 이미 저장된 기록을 실패로 되돌리지 않습니다.
- 새 `%ProgramData%\SamsungSwitchWatch` 루트는 Setup이 원자적 생성에 성공한 경우에만 제품
  소유 폴더로 인정합니다. 검사와 생성 사이에 다른 프로세스가 같은 폴더를 만들면 그 폴더의
  ACL이나 내용을 변경하지 않고 `SETUP_PATH_UNTRUSTED`로 중단합니다.
...
`0.11.5-poc`에서 서비스 설명, 자동 복구 정책 또는 DACL 복원만 실패하면 각각
`ROLLBACK_SERVICE_DESCRIPTION_RESTORE_WARNING`,
`ROLLBACK_SERVICE_RECOVERY_POLICY_RESTORE_WARNING`,
`ROLLBACK_SERVICE_DACL_RESTORE_WARNING` 경고가 남을 수 있습니다. 서비스 핵심 상태가 다시
확인되면 복구는 완료되고 작업 기록은 정리됩니다. 반대로 서비스 실행 파일 경로, 시작 유형,
계정, 표시 이름, 서비스 SID 또는 이전 실행 상태를 복원하지 못하면
`ROLLBACK_SERVICE_RESTORE_FAILED`로 중단하며 작업 기록과 이전 파일을 보존합니다. 이 경우
제품 폴더나 작업 기록을 수동으로 삭제하지 말고 지원 코드 또는 진단 요약을 전달하십시오.
...
먼저 `로그인 확인`을 실행합니다. 이 단계는 TCP/23, 계정, 선택적 enable과 최종 프롬프트를
확인한 뒤 `show version`을 한 번 실행하여 모델을 자동 판별합니다. 판별 원문은 화면·설정·로그에
저장하지 않고 지원되는 모델명만 Viewer로 반환합니다. 새 장비는 판별 성공 후 저장할 수 있습니다.
지원되는 모델을 찾지 못하면 `MODEL_NOT_DETECTED`, 둘 이상의 지원 모델 토큰이 함께 보이면
`MODEL_AMBIGUOUS`로 중단합니다. 두 경우 모두 임의 모델을 선택하지 말고 장비의 실제 버전과
지원 범위를 확인합니다.
구형 Agent가 판별 결과를 제공하지 않으면 `MODEL_DETECTION_UNAVAILABLE`을 표시하므로 Agent와
Viewer를 같은 최신 버전으로 업데이트합니다. 실제 포트·로그 명령은 수집 진단 또는 수동 명령에서
확인합니다. 수동 입력은 한 줄
`show` 명령 하나만 허용하며 Viewer와 Agent가 각각 다시 검증합니다.

`포트 상태/시스템 로그 수동 점검`과 직접 입력한 읽기 전용 명령은 Viewer 전체에서 동시에 하나만 실행됩니다. 하나가 실행
중이면 다른 실행 버튼은 잠시 비활성화되며, 뒤에서 대기열로 중복 실행되지 않습니다. 장비 관리
창을 닫으면 진행 중인 로그인 확인을 취소하고 화면에 입력된 로그인 PW와 enable PW를 즉시
지웁니다. 창을 닫은 뒤 늦게 도착한 결과로 닫힌 화면이 갱신되지 않습니다.
...
- Agent 연결을 바꾸거나 Viewer를 종료할 때 진행 중인 요청을 취소한 뒤 제한된 시간 안에서
  이전 연결 자원을 정리합니다. 연결 교체나 종료로 발생한 정상적인 요청 취소는 앱 오류나 연결
  실패로 다시 표시하지 않습니다. 정리 실패는 사용자용 상태와 비식별 진단 로그에서 구분합니다.
...
같은 수집 항목에 여러 읽기 전용 후보 명령이 있으면 `COMMAND_TIMEOUT` 또는 `QUERY_TIMEOUT`
발생 직후 같은 점검에서 재접속하지 않습니다. 다음 감시 주기에 다음 후보를 한 번 시도하고,
마지막 후보까지 실패하면 해당 후보를 유지한 채 `확인 불가`로 표시합니다. 이는 짧은 세션 제한
장비에 연속 로그인을 발생시키지 않기 위한 동작입니다.
