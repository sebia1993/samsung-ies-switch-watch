...
python .\tools\render-user-manual-pdf.py `
  ...
  --render-dir .\tmp\manual-render-0.11.8
```
...
```powershell
.\scripts\build-release.ps1 -Version 0.11.8-poc -AllowDirty
```
...
RELEASE_NOTES_0.11.8_POC_KO.md
BUILD-MANIFEST.json
SBOM.spdx.json
SBOM.cdx.json
```
...
```powershell
git tag -a v0.11.8-poc -m "Samsung Switch Watch v0.11.8-poc"
git push origin v0.11.8-poc
```
