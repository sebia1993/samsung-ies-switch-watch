...
- Windows CI의 deterministic 5분 stability smoke가 queue/worker 상한과 clean shutdown을 통과함
...
- Viewer Setup이 검증되지 않은 기존 프로그램 폴더를 transaction backup으로 옮기고, commit 전
  실패·취소 시 원래 설치 경로로 복원하며 commit 뒤에만 최근 제품 격리본 1개로 확정함
- Viewer Setup이 새 journal format 3을 기록하고 기존 format 2를 원래 의미대로 복구함
- format 3 복구가 남으면 v0.12.0 또는 더 최신 Setup으로 복구를 끝낸 뒤에만 downgrade함
- 프로그램 격리·복구 중 `%LOCALAPPDATA%\SamsungSwitchWatch`의 장비·DPAPI·감시 데이터가 바뀌지 않음
- 미완료 Agent 트랜잭션에서 설치가 차단되고 별도 `이전 상태 복구`만 허용됨
- 기존 Agent→backup과 staging→설치 위치의 활성화 이동이 각각 최대 5회로 제한되고,
  실패한 시도 사이에만 250ms 대기함
- 지속적인 이동 실패가 `SETUP_BACKUP_MOVE_FAILED` 또는
  `SETUP_FILE_ACTIVATION_FAILED`로 안정적으로 분류되고 기존 rollback을 수행함
- backup 관리자 전용 ACL 강화 실패는 `SETUP_BACKUP_ACCESS_WARNING` 경고로 남기되
  파일·서비스·준비 상태가 정상인 설치를 실패로 바꾸지 않음
- 서비스 상태 snapshot 실패가 `SETUP_SERVICE_CAPTURE_FAILED`, 기존 서비스 계약 불일치가
  `SETUP_SERVICE_CONTRACT_FAILED`, 중지 실패가 `SETUP_SERVICE_STOP_FAILED`, 핵심 구성 실패가
  `SETUP_SERVICE_CONFIG_FAILED`, 시작 실패가 `SETUP_SERVICE_START_FAILED`로 구분됨
- 서비스 실행 파일 경로·자동 시작 유형·가상 계정과 실제 Running 확인은 fail-closed로 유지됨
- 서비스 설명·자동 복구 정책·제한 DACL 적용만 실패하면
  `SETUP_SERVICE_DESCRIPTION_WARNING`, `SETUP_SERVICE_RECOVERY_POLICY_WARNING`,
  `SETUP_SERVICE_DACL_WARNING`으로 구분하고 핵심 서비스 구동이 확인된 설치를 되돌리지 않음
...
- 취소·중복 실행을 제외한 조치 가능한 Viewer Setup 실패와 복구 불가 검사에서만 별도 SWS1
  지원 코드가 표시되고 새 작업·성공·취소 때 지워짐
- SWS1이 24자 고정 형식과 CRC 검사를 사용하며 경로·사용자·파일 해시·transaction ID·자격 증명·
  장비 정보·예외 원문을 포함하지 않음
- 기존 SWD1 고정 벡터와 Agent Setup·Viewer 연결 해석 결과가 변경되지 않음
- 공개 readiness 응답이 Agent ID·SPKI·token·장비 정보를 포함하지 않음
- production Agent가 DPAPI LocalMachine으로 보호한 영구 RSA 인증서와 32-byte token을
  재시작 뒤에도 유지하고, Viewer는 수동 페어링한 SPKI pin과 bearer를 모두 검증함
- token·pin 불일치, 인증서 EKU·유효기간 오류, API v4는 자동 fallback 없이 차단됨
...
- 회귀 테스트 대상: Viewer Setup의 invalid 기존 설치가 모든 pre-commit 실패·취소에서 원래
  경로로 복원되고 commit·격리 회전 각 단계에서 중단돼도 journal로 재개되는지 확인
- 회귀 테스트 대상: 기존 format 2와 신규 format 3 Viewer journal 복구 및 구형 Setup의
  format 3 fail-closed 동작 확인
- API v5 인증·재페어링 마이그레이션, Viewer 설정 보존과 v4 426 계약이 검증됨
...
- 취소·중복 실행을 제외한 조치 가능한 Viewer Setup 실패와 복구 불가 검사의 선택·복사 가능한
  SWS1 및 자동 격리·원복·복구 대기 안내
- SWS1과 기존 SWD1의 대상 차이, 민감정보 제외 범위와 downgrade 복구 조건
...
python .\tools\render-user-manual-pdf.py `
  ...
  --render-dir .\tmp\manual-render-0.13.0-poc
```
...
```powershell
.\scripts\build-release.ps1 -Version 0.13.0-poc -AllowDirty
```
...
RELEASE_NOTES_0.11.11_POC_KO.md
BUILD-MANIFEST.json
SBOM.spdx.json
SBOM.cdx.json
```
...
```powershell
git tag -a v0.13.0-poc -m "Samsung Switch Watch v0.13.0-poc"
git push origin v0.13.0-poc
```
